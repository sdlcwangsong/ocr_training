...
#这个要根据实际系统进行修改
FONT_PATH = "/System/Library/Frameworks/Python.framework/Versions/3.5/lib/python3.5/site-packages/matplotlib/mpl-data/fonts/ttf/VeraMoBd.ttf"
...
class picChecker():
    ...
    def __init__(self, chars = initChars(),
            size = (250, 100), fontsize = 37,
            begin = (17, 17), outputType = 'TIFF',
            mode = 'RGB', backgroundColor = (255, 255, 255),
            foregroundColor = (255, 0, 0),
            fonttype = "wqy-microhei.ttc", length = 4, jamNum = (0, 0),
            pointBorder = (0, 99999999999)
            ):
        self.chars = chars
        self.size = size
        self.begin = begin
        ...
        #self.font = PIL.ImageFont.truetype(self.fonttype, self.fontsize)
        self.font = PIL.ImageFont.truetype(FONT_PATH, self.fontsize)
    ...
    def createChecker(self, filename):
        """
        usage: createChecker()
        return: [str,pic]
        str:对应的字符串
        ...
        im = PIL.Image.new(self.mode, self.size, self.backgroundColor)
        ...
        im = im.transform(im.size, PIL.Image.PERSPECTIVE, para)
        im = im.filter(PIL.ImageFilter.EDGE_ENHANCE_MORE)
        im.save(filename, self.outputType)
        return ([randStr, im])

if __name__ == "__main__":
    c = picChecker()
    t = c.createChecker("training/mon.mourney.exp0.tif")
    print(t[0])
